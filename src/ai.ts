...
async function runTextModel(env: Env, messages: Array<{ role: string; content: string }>): Promise<string> {
...
export async function summarizeInvestigation(env: Env, query: string, sources: ScrapedSource[]): Promise<{ summary: string; riskLevel: string }> {
  const evidence = sources.map((source, index) => ({ source: index + 1, title: safeText(source.title, 240), url: source.url, text: safeText(source.text, 8_000) }));
  const response = await runTextModel(env, [
  ...
  try {
    const parsed = JSON.parse(response) as { summary?: unknown; riskLevel?: unknown };
