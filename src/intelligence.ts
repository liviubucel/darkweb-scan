...
export async function tenantNamespace(orgId: string): Promise<string> {
  ...
  return `zbt-${btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "")}`;
}

function embeddingText(source: ScrapedSource): string {
  const normalized = `${source.title}\n${source.text}`
    .replace(/[\u0000-\u001f]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const words = normalized.split(" ").slice(0, 220).join(" ");
  return words.slice(0, 1_600);
}
...
export async function persistEvidence(env: Env, orgId: string, investigationId: string, sourceId: string, source: ScrapedSource): Promise<string> {
  const prefix = await tenantNamespace(orgId);
  ...
  await env.EVIDENCE.put(key, JSON.stringify({ version: 1, investigationId, sourceId, title: source.title, onionUrl: source.url, text: source.text, contentType: source.contentType, fetchedAt: source.fetchedAt, sha256: source.sha256 }), {
    ...
    customMetadata: { investigationId, sourceId, contentSha256: source.sha256 },
  });
  return key;
}
...
export async function indexSource(env: Env, orgId: string, investigationId: string, sourceId: string, source: ScrapedSource): Promise<void> {
  ...
  const text = embeddingText(source);
  if (!text) return;
  ...
  await env.INTELLIGENCE_INDEX.upsert([{
    id: sourceId,
    namespace: await tenantNamespace(orgId),
    values,
    metadata: { investigationId, sourceId, fetchedAt: source.fetchedAt },
  }]);
}
